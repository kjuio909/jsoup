...
/**
 ...
 Additionally, <code>:nth-child()</code> supports <code>odd</code> and <code>even</code> as arguments. <code>odd</code> is the same as <code>2n+1</code>, and <code>even</code> is the same as <code>2n</code>.
 <p>As of Selectors Level 4, an optional <code>of &lt;selector-list&gt;</code> clause may be supplied: only the element children of the same parent that match the list (the element itself included) count toward the 1-based index; other siblings do not occupy a position. The selector list uses the normal element selector syntax, including comma lists and nested pseudo selectors.</p></td><td><code>tr:nth-child(2n+1)</code> finds every odd row of a table. <code>:nth-child(10n-1)</code> the 9th, 19th, 29th, etc, element. <code>li:nth-child(5)</code> the 5h li. <code>i:nth-child(2n of .x)</code> every other <code>i</code> among its <code>.x</code> siblings.</td></tr>
 <tr><td><code>:nth-last-child(<em>a</em>n+<em>b</em>)</code></td><td>elements that have <code><em>a</em>n+<em>b</em>-1</code> siblings <b>after</b> it in the document tree. Otherwise like <code>:nth-child()</code>, including the optional <code>of &lt;selector-list&gt;</code> clause, where positions are counted from the end of the filtered siblings.</td><td><code>tr:nth-last-child(-n+2)</code> the last two rows of a table; <code>:nth-last-child(2n of .x)</code> every other match counting from the end</td></tr>
