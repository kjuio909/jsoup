...
/**
 ...
 <tr><td><code>:lang(<em>language</em>)</code></td><td>elements whose language matches the given language range. An element's language comes from its own <code>lang</code> attribute, or the nearest inherited <code>lang</code> attribute up the parent chain; an empty <code>lang</code> value means the language is unknown and stops inheritance. The range matches when the language is equal to it, or starts with it followed by a <code>-</code> (so <code>en</code> matches <code>en-US</code>, but not <code>enough</code>). The comparison is ASCII case-insensitive. The range may be bare or quoted.</td><td><code>p:lang(en)</code> finds English paragraphs, including <code>en-US</code></td></tr>
