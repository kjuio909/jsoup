...
/**
 ...
 As of Selectors Level 4, an optional <code>of &lt;selector-list&gt;</code> clause is supported: only the element siblings that match the selector list are considered when computing the <em>a</em>n+<em>b</em> position; other siblings do not hold a position, and the element under test must itself match the list. The list may contain commas and nested pseudo selectors. (<code>:nth-of-type()</code> does not support this clause.)</td><td><code>tr:nth-child(2n+1)</code> finds every odd row of a table. <code>:nth-child(10n-1)</code> the 9th, 19th, 29th, etc, element. <code>li:nth-child(5)</code> the 5h li. <code>:nth-child(2n of .highlight)</code> every second sibling carrying the <code>highlight</code> class</td></tr>
 <tr><td><code>:nth-last-child(<em>a</em>n+<em>b</em>)</code></td><td>elements that have <code><em>a</em>n+<em>b</em>-1</code> siblings <b>after</b> it in the document tree. Otherwise like <code>:nth-child()</code>, and also supports the <code>of &lt;selector-list&gt;</code> clause, counting positions from the end of the filtered siblings</td><td><code>tr:nth-last-child(-n+2)</code> the last two rows of a table</td></tr>
