...
/**
 ...
 <tr><td><code>:nth-child(<em>a</em>n+<em>b</em> of <em>S</em>)</code></td><td><p>as <code>:nth-child()</code>, but the index is counted only among the element siblings that match the selector list <em>S</em>. Only direct element children of the same parent are considered; non-element nodes (such as text and comments) are never counted. The element being tested must itself match <em>S</em>, and elements without a parent do not match. Counting starts at 1, in document order for <code>:nth-child()</code> or from the last sibling for <code>:nth-last-child()</code>.</p></td><td><code>li:nth-child(2 of .highlight)</code> finds the second <code>li</code> with class <code>highlight</code> among its siblings.</td></tr>
 <tr><td><code>:nth-last-child(<em>a</em>n+<em>b</em>)</code></td><td>elements that have <code><em>a</em>n+<em>b</em>-1</code> siblings <b>after</b> it in the document tree. Otherwise like <code>:nth-child()</code>, and also supports the <code>of <em>S</em></code> clause</td><td><code>tr:nth-last-child(-n+2)</code> the last two rows of a table; <code>li:nth-last-child(1 of .highlight)</code> the last highlighted <code>li</code></td></tr>
