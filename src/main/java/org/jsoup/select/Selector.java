...
/**
 ...
 <tr><td><code>:scope</code></td><td>The scope (context) element of the evaluation. Within a <code>:has()</code> condition, this is the element the condition is being tested on, so <code>:has(&gt; p)</code> and <code>:has(:scope &gt; p)</code> are equivalent. In a top-level query, it is the element the query was started from.</td><td><code>div:has(:scope &gt; p)</code> finds <code>div</code>s with a direct child <code>p</code></td></tr>
