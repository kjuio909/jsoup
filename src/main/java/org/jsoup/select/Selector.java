...
/**
 ...
 <tr><td><code>:lang(<em>language</em>)</code></td><td>elements whose language matches the range <em>language</em>, via their own <code>lang</code> attribute or the nearest ancestor carrying one (an empty <code>lang</code> attribute means the language is unknown and blocks inheritance). Matching is ASCII case-insensitive and requires an exact match or the range followed by a hyphen, so <code>:lang(en)</code> matches <code>en</code> and <code>en-US</code> but not <code>enough</code>.</td><td><code>p:lang(fr)</code> finds paragraphs in French; <code>html:lang(en)</code> matches the <code>html</code> element carrying <code>lang="en"</code> and its descendants, up to a nested <code>lang</code> boundary.</td></tr>
 ...
 <tr><td><code>:scope</code></td><td>The element that the selector is evaluated against: the root element of a <code>select()</code> call, or the anchor element of a <code>:has()</code> clause. Equivalent to the implicit anchor used by a leading combinator, so <code>:has(&gt; p)</code> and <code>:has(:scope &gt; p)</code> are the same.</td><td><code>article:has(:scope &gt; p &gt; b + i)</code> finds <code>article</code> elements with a child <code>p</code> containing a <code>b</code> immediately followed by an <code>i</code></td></tr>
 ...
 Additionally, <code>:nth-child()</code> supports <code>odd</code> and <code>even</code> as arguments. <code>odd</code> is the same as <code>2n+1</code>, and <code>even</code> is the same as <code>2n</code>.</p>
 <p>An optional <code>of S</code> clause may be supplied, where <code>S</code> is a comma-separated list of complex selectors: <code>:nth-child(An+B of S)</code>. Only element siblings matching <code>S</code> are candidates and counted; the position is then the index among those matching siblings, in document order. E.g. <code>:nth-child(2 of b)</code> matches the second <code>b</code> sibling, regardless of other siblings between them.</p></td><td><code>tr:nth-child(2n+1)</code> finds every odd row of a table. <code>:nth-child(10n-1)</code> the 9th, 19th, 29th, etc, element. <code>li:nth-child(5)</code> the 5h li. <code>:nth-child(2 of .item)</code> the second sibling with class <code>item</code></td></tr>
 <tr><td><code>:nth-last-child(<em>a</em>n+<em>b</em>)</code></td><td>elements that have <code><em>a</em>n+<em>b</em>-1</code> siblings <b>after</b> it in the document tree. Otherwise like <code>:nth-child()</code>, including the optional <code>of S</code> clause (counted in reverse document order)</td><td><code>tr:nth-last-child(-n+2)</code> the last two rows of a table; <code>:nth-last-child(1 of b, i)</code> the last <code>b</code> or <code>i</code> sibling</td></tr>
