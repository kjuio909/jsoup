...
/**
 ...
 <p>The optional <code>of S</code> clause (a <a href="https://www.w3.org/TR/selectors-4/#nth-child-pseudo">Selectors Level 4</a> feature) restricts the counting to the element siblings matching selector <code>S</code>: only those siblings are numbered from 1 (non-element nodes are never counted), and an element matches only if it matches <code>S</code> itself. The selector <code>S</code> is a selector list, and may itself contain nested pseudo selectors.</p></td><td><code>tr:nth-child(2n+1)</code> finds every odd row of a table. <code>:nth-child(10n-1)</code> the 9th, 19th, 29th, etc, element. <code>li:nth-child(5)</code> the 5h li. <code>li:nth-child(2 of .highlight)</code> the second <code>li</code> sibling with the {@code highlight} class</td></tr>
 <tr><td><code>:nth-last-child(<em>a</em>n+<em>b</em> [of <em>S</em>])</code></td><td>elements that have <code><em>a</em>n+<em>b</em>-1</code> siblings <b>after</b> it in the document tree. Otherwise like <code>:nth-child()</code>, including the optional <code>of S</code> clause (counted in reverse, starting from 1).</td><td><code>tr:nth-last-child(-n+2)</code> the last two rows of a table. <code>li:nth-last-child(1 of .highlight)</code> the last <code>li</code> sibling with the {@code highlight} class</td></tr>
