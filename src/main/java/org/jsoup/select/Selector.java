...
/**
 ...
 The form <code>:nth-child(<em>a</em>n+<em>b</em> of S)</code> is also supported, where <code>S</code> is a comma-separated list of selectors: the candidate and the siblings that are indexed are only those that match <code>S</code>, in document order.</td><td><code>tr:nth-child(2n+1)</code> finds every odd row of a table. <code>:nth-child(10n-1)</code> the 9th, 19th, 29th, etc, element. <code>li:nth-child(5)</code> the 5h li. <code>:nth-child(2 of .item)</code> the second sibling with class {@code item}</td></tr>
 <tr><td><code>:nth-last-child(<em>a</em>n+<em>b</em>)</code></td><td>elements that have <code><em>a</em>n+<em>b</em>-1</code> siblings <b>after</b> it in the document tree. Otherwise like <code>:nth-child()</code>; the <code>of S</code> form is supported too, with siblings indexed in reverse document order</td><td><code>tr:nth-last-child(-n+2)</code> the last two rows of a table</td></tr>
