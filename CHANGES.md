...
* Reworked CSS attribute selector value handling to follow CSS more closely. Attribute names may use CSS escapes (e.g. `[\64 ata]` is `[data]`) and remain case-insensitive. Quoted values decode CSS string escapes (e.g. `[data-v='x\22 y']`), and may contain whitespace, `=`, and the other quote type; unquoted values end at whitespace. Whitespace is allowed around the value operator. Value comparisons are now case-sensitive by default; append `i` after the value to ignore case, or `s` to require case-sensitivity (e.g. `[type=text i]`). The `!=` operator is the exact complement of `=`, so elements missing the attribute match, while an empty value remains distinct from a missing attribute. Malformed selectors (unclosed brackets or quotes, whitespace in unquoted values, unknown or repeated case modifiers, incomplete escape sequences) now consistently raise a `SelectorParseException`.
