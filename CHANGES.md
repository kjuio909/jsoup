...
* Added `Nodes#outerHtml(Appendable)` (inherited by `Elements`), which streams each node's outer HTML to the supplied `Appendable` in iteration order, separated by a single newline, rather than building the combined string in memory. Each node is rendered through its own `Node#outerHtml(Appendable)` with that node's document output settings, mixed node types and duplicate references are written as encountered, the appendable's existing contents are preserved, and an `IOException` is surfaced as a `SerializationException` that stops output while leaving the already-written prefix intact. The string `outerHtml()` and `toString()` results are unchanged and are now produced by the same path.
