...
### Bug Fixes
* Improved `FormElement.formData()` to follow the browser rules for successful controls. Controls are collected in document order, including controls associated to the form by a `form` attribute (and excluding those associated to another form or carrying an unresolvable `form` reference); controls within a disabled `fieldset` are excluded, except those within its first `legend`; disabled `option`s are not submitted; a single `select` submits one selected enabled option (falling back to the first enabled option when none is selected), while a `multiple` select submits all selected enabled options and submits nothing when none qualify.
