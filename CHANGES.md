...
### Improvements
* Added `ProcessingInstruction` node support to the XML parser. `<?target data?>` processing instructions are now parsed into dedicated nodes that preserve their target and data (including `>` characters and non-ASCII content), instead of being treated as pseudo `XmlDeclaration` tags. The data ends only at the `?>` closer, so instructions no longer swallow or corrupt adjacent nodes, and unterminated instructions are dropped cleanly. The instruction data can be read and updated via `data()`, and is parsed into attributes on demand using the owning document's parser settings. `<?xml ...?>` declarations, `<!NAME ...>` markup declarations, doctypes, comments, and CDATA sections keep their existing representations, and HTML parsing is unchanged. [#2613](https://github.com/jhy/jsoup/pull/2613)
* Added `Parser#parseAttributes(String)`, which parses attribute names and values without a surrounding tag, using the parser's settings. [#2613](https://github.com/jhy/jsoup/pull/2613)
* Added `Nodes#outerHtml(Appendable)` (inherited by `Elements`), which writes the outer HTML of each node directly to the supplied `Appendable` in iteration order, separated by a single newline, without first concatenating the entire result. This enables true streaming for large-document extraction, paginated transport, and log pipes. Existing content in the appendable is preserved, no newline is written before the first node, an empty list writes nothing, and duplicate node references are each written in turn. Each node uses its own document's output settings (syntax, charset, `prettyPrint`, entity escaping), so nodes from different documents do not affect one another and detached nodes keep the existing defaults. An `IOException` from the appendable is reported as a `SerializationException` with the original cause; output stops immediately and no further separator or node is written, while the prefix already written remains in place. Writing into a `StringBuilder` is character-for-character identical to `Nodes#outerHtml()` and `#toString()`, whose behavior is unchanged.
