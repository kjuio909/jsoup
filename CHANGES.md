...
* Added support for the `:lang(R)` CSS pseudo-class. An element matches when its own `lang` attribute, or the nearest ancestor's `lang` attribute when it has none, equals the range `R` or begins with `R` followed by a hyphen; comparison is ASCII case-insensitive (so `:lang(en)` matches `en` and `en-US` but not `enough`). A present-but-empty `lang` attribute denotes an unknown language and blocks further inheritance. The range may be bare or quoted, with whitespace allowed only outside it; empty arguments, internal whitespace, comma lists, unbalanced quotes or parentheses, and trailing characters are parse errors.
