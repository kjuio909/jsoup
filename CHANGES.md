...
### Bug Fixes
* Fixed CSS hexadecimal escapes in unquoted attribute selector values ending the value at the escape's own terminating whitespace, so e.g. `[data-v=a\3d b]` now correctly matches the value `a=b`. An escape of whitespace or a dangling escape in an unquoted value remains a parse error.
