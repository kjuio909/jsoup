...
### Improvements
* Added support for the `:lang(R)` CSS pseudo-class, matching elements whose language (from their own or nearest inherited `lang` attribute) equals the range `R` or starts with `R` followed by a `-`, compared ASCII case-insensitively (so `:lang(en)` matches `en-US` but not `enough`). An empty `lang` value means the language is unknown and stops inheritance. The range may be bare or quoted, with optional whitespace just inside the parentheses; an empty range, whitespace or commas within the range, unclosed quotes or parentheses, or trailing content after the range are parse errors.
* Added support for the CSS case-sensitivity modifiers `i` and `s` in attribute selectors (e.g. `[attr="val" i]`, `[attr=val s]`). Quoted (CSS string) attribute values now compare case-sensitively by default and decode CSS backslash and hexadecimal escapes (e.g. `[data-v='x\22 y']`); unquoted values keep jsoup's historical case-insensitive comparison and run to the next whitespace or modifier. CSS escapes in attribute names are decoded before matching, so e.g. `[\64 ata]` and `[d\61 ta]` are synonymous with `[data]`. Whitespace is allowed around the comparison operators. Unquoted values containing whitespace, unknown or repeated modifiers, modifiers not following a value, unclosed quotes or brackets, and incomplete escape sequences are parse errors.
...
### Bug Fixes
* Aligned `FormElement.formData()` with the browser rules for successful controls. Controls are collected in document order across the whole document, including elements associated with the form via a `form` attribute (and excluding those referencing another or an unknown form); controls inside a disabled `fieldset` are excluded, except those within its first `legend`; disabled `option`s are skipped; a single-value `select` submits only its first selected enabled option, falling back to the first enabled option when none is selected; and a `multiple` select submits all of its selected enabled options. `FormElement.elements()` now returns the associated controls in document order, without duplicates when parse error recovery has moved controls out of the form.
* The `!=` attribute selector is now the exact complement of `=`: an element matches unless it has the attribute and its value equals, so elements missing the attribute always match (including when comparing against an empty value, which remains distinct from a missing attribute).
